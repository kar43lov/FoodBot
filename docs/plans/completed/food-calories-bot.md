...
- [x] Компонент календаря (неделя по умолчанию)
- [x] Переключение неделя/месяц
- [x] Строки: пользователи проекта
- [x] Столбцы: дни
- [x] Ячейки: записи + сумма ккал за день
- [x] Клик на ячейку → детали записей

### Task 12: Web App — CRUD операции

- [x] Модальное окно добавления записи (дата, время, калории, описание)
- [x] Редактирование существующей записи
- [x] Удаление записи с подтверждением
- [x] Проверка прав: свои записи или админ
...
- [x] Комментарии к сложным частям кода
- [x] API документация (Swagger)

### Task 15: Тестирование и финализация

- [x] Unit-тесты для AI модуля
- [x] Integration-тесты для API endpoints
- [x] E2E тест: фото → распознавание → сохранение
- [x] Проверка Definition of Done:
  - [x] Бот распознаёт еду
  - [x] Сохраняет калории
  - [x] Работает polling + webhook
  - [x] Веб открывается через Telegram
  - [x] Календарь показывает данные
  - [x] CRUD работает
  - [x] README есть
